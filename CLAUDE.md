...
- **Game mode**: set by cvars `game_type` + `game_mode` *together* (game_mode's meaning depends on game_type), applied only on a **map reload**. Combos: Casual 0/0, Competitive 0/1, Wingman 0/2, ArmsRace 1/0, Demolition 1/1, **Deathmatch 1/2**. Boot default is **Deathmatch on de_dust2** (set in the unit's `+game_type 1 +game_mode 2 +map de_dust2`). **Presets**: `game/csgo/cfg/mode_*.cfg` (dm/comp/casual/wingman/armsrace/demolition) each set the cvars + a `changelevel` (de_dust2, except **wingman → de_sanctum**); wired into SimpleAdmin's `CustomServerCommands` so they appear in **`!admin` → Custom Commands** (one click switches mode; or `css_rcon exec mode_dm`). SimpleAdmin's `CS2-SimpleAdmin.json` is **JSONC** (leading `//` header) — strip the comment line before parsing with a JSON tool.
